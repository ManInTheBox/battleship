defmodule Battleship.ShipTest do
  ...
  test "creates new Submarine" do
    assert %Battleship.Ship{squares: [{1, 1}], type: :submarine} = Battleship.Ship.new({1, 1})
  end

  test "creates new Destroyer" do
    assert %Battleship.Ship{squares: [{1, 1}, {2, 1}], type: :destroyer} = Battleship.Ship.new({2, 1}, {1, 1})
  end

  test "creates new Cruiser" do
    assert %Battleship.Ship{squares: [{3, 4}, {4, 4}, {5, 4}], type: :cruiser} = Battleship.Ship.new({5, 4}, {3, 4}, {4, 4})
  end

  test "creates new Battleship" do
    assert %Battleship.Ship{squares: [{7, 10}, {8, 10}, {9, 10}, {10, 10}], type: :battleship} = Battleship.Ship.new({10, 10}, {7, 10}, {8, 10}, {9, 10})
  end
